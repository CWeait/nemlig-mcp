...
class ConfigTest {
    ...
    fun `should use default values when environment variables not set`() {
        ...
        assertEquals("https://webapi.prod.knl.nemlig.it", config.nemlig.apiUrl)
    ...
    fun `should have rate limit configuration`() {
        ...
        assertEquals(5, config.nemlig.rateLimit.requestsPerSecond)
        assertEquals(10, config.nemlig.rateLimit.burstSize)
    }
}
